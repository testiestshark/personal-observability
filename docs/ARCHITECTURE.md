...
**Hosted env vars are configured separately from git, in Lovable's own environment/Supabase-connection panel — not from the committed `.env`/`.env.example`.** Discovered 2026-08-02: after merging the auth feature, hosted signup failed with a missing-env-var error because `SUPABASE_URL`/`SUPABASE_PUBLISHABLE_KEY` were never set there. When adding or rotating these, source the values from the *hosted* project's dashboard (Project Settings → API on project `ivdhucdiycnbgvdetagw`) — never copy from `.env.local`, which points at the local stack. `SUPABASE_PUBLISHABLE_KEY`/`VITE_SUPABASE_PUBLISHABLE_KEY` are safe to expose (protected by RLS, not secrecy); `SUPABASE_SERVICE_ROLE_KEY` is not needed for auth and shouldn't be added until something server-side actually requires it.
...
**Generated scaffolding — do not re-register it.** `src/integrations/supabase/client.ts`, `auth-middleware.ts` and `auth-attacher.ts` are Lovable-generated (`// This file is automatically generated`) and implement a different, `localStorage` + `Authorization: Bearer` approach. Don't build on them; use `src/lib/auth/` instead. They are left in place rather than edited, since hand-editing generated files risks silent breakage when Lovable regenerates them.

`attachSupabaseAuth` **must not** be registered as `functionMiddleware` in `src/start.ts`. It is a `.client()` middleware, so it runs in the *browser* on every server-function call and eagerly constructs the generated Supabase client, which throws when `VITE_SUPABASE_*` are missing from the client bundle. That is a production-only failure: local `.env.local` defines those variables, so it stays silent locally and breaks only once deployed. It caused hosted sign-in to fail with a generic "Something went wrong" on 2026-08-02 (fixed in `fix/remove-bearer-middleware`). Registering it also pulls the whole Supabase auth library into the browser bundle, defeating the httpOnly-cookie design above.
