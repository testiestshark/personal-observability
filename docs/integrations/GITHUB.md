...
Status: design only. No code, migrations, or GitHub configuration exist yet.
...
- **Supabase Auth** — establishes *who the PO user is* (see §4). The connect route must run behind `requireSupabaseAuth`-equivalent session validation so the state can be bound to a real user.
...
- **Blocking dependency:** Personal Observability has no login/session system implemented yet (`requireSupabaseAuth` exists but is unused, no login route exists). This milestone assumes a PO session already exists to bind `state` to — that assumption itself needs to be resolved first, and isn't something this document can respons­ibly settle from the current repository evidence.
