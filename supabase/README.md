...
- **`with check` on insert/update** — `using` alone only filters what you can *see*; `with check` is what stops a request from *writing* a row it shouldn't own. Insert has no existing row to filter on, so `using` wouldn't apply there at all — `with check` is the only gate available for insert.
...
| 2 | Which *rows* may it see? | RLS policy | The role can touch the table, but sees/changes nothing (or everything, if RLS itself is off) |
